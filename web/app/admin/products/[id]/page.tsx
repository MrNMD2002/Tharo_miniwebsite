...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    React.useEffect(() => {
        const fetchProduct = async () => {
            try {
                ...
                setToast({ isVisible: true, message: "Không tìm thấy sản phẩm", type: "error" });
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleImagesChange = (urls: string[]) => {
        setFormData((prev) => ({ ...prev, images: urls.slice(0, 5) }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setToast({ isVisible: true, message: "Đã cập nhật sản phẩm!", type: "success" });
            setTimeout(() => router.push("/admin/products"), 1500);
        } catch (error) {
            console.error("Error updating product:", error);
            setToast({ isVisible: true, message: "Có lỗi xảy ra khi cập nhật", type: "error" });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            />
            <h1 className="text-2xl font-serif font-bold text-burgundy-900 mb-6">Chỉnh sửa sản phẩm</h1>
                    ...
                    <div>
                        ...
                        <label className="inline-flex items-center cursor-pointer">
                            ...
                            <span className="ms-3 text-sm font-medium text-gray-700">
                                {formData.isActive !== false ? "Hiện" : "Ẩn"}
                            </span>
                        </label>
                        <p className="mt-1 text-xs text-gray-500">
                            {formData.isActive !== false ? "Sản phẩm sẽ hiển thị trên trang chủ và trang sản phẩm" : "Sản phẩm sẽ bị ẩn trên site (chỉ admin thấy)"}
                        </p>
                    </div>
                </div>
